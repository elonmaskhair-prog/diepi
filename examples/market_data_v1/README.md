...
这里是一份可以被 CLI 和 GUI 直接读取的教学数据根目录：`data/`。项目维护者
从公开渠道收集的本地行情库中，按固定范围只读切片并规范化了以下内容：
...
这些 Parquet 是上述证券的真实历史行情切片，不是合成走势。维护者决定继续随源码和
sdist 提供它们，并负责保存、复核来源条款和必要证据。但“公开渠道可取得”“个人使用”或
“未盈利”本身不是第三方再分发授权证明；项目的 Apache-2.0 代码许可证也不会自动授予
行情数据权利。需要复制或再次发布本目录的下游使用者必须自行核对适用来源、条款和权限。
