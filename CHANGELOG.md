...
本文件记录 dieΠ 的用户可见变化。当前公开版本仍是 Alpha；**未发布**下的内容描述
下一候选，不代表已经发布或形成稳定兼容性保证。
...
- 新增版本化 `diepi.integration` 公共门面与 capability 握手，供本地 Agent/MCP 等编排
  adapter 在接收任务前一次性验证停止回调、来源指纹、日历、数据校验和工件绑定能力。

### 安全

- dataset manifest、manifest 声明的 Parquet、证券元数据和本地交易日历 override 统一使用
  有 containment、link/junction/reparse 拒绝、稳定句柄和字节上限的普通文件读取边界。
- manifest 路径增加跨 Windows/POSIX 的保留名、ADS、尾点/空格与大小写冲突拒绝；正式
  CLI 在执行前核对全部 manifest 逻辑身份，并对实际价格轨、证券 metadata 与已知辅助时序
  做运行前后稳定字节指纹，避免运行可达输入变化绕过来源绑定。
- 补齐停止回调、日历祖先目录和直接来源证据契约的公共回归测试，并纳入精确发布白名单。

### 变更

- 发行版本推进到 `0.1.1`；补齐 GitHub 项目、问题、变更记录和安全政策元数据。
- 明确公开真实教学切片的来源/再分发证据仍由维护者负责：公开可访问和非商业使用本身
  不是第三方再分发授权证明，Apache-2.0 代码许可也不会自动覆盖行情数据权利。
...
- 项目仍为 Alpha 软件。
- 首个公开源码和 PyPI 版本于 2026-08-18 发布。
- 尚未承诺长期支持周期；重要研究仍须记录精确版本、Git commit 或安装工件 hash。
